...
@WebServlet("/UpdateInfoServiceCon")
public class UpdateInfoServiceCon extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		InfoDTO dto = new InfoDTO(filename, m_id, title1, q1, a1, q2, a2, q3, a3, q4, a4);
		InfoDTO sdto = new InfoDTO(m_id, title1, school, g_date);
		InfoDTO ldto = new InfoDTO(m_id, title1, lisence, getdate, place);
		...
		if (icnt > 0 && scnt > 0 && lcnt > 0) {
			...
			if (icnt > 0) {
				...
				if (scnt < 1) {
					System.out.println("school가입 실패");
				}
				if (lcnt > 0) {
		...
		if (icnt > 0 && scnt > 0 && lcnt > 0) {
			System.out.println("자소서 수정 성공");
		} else {
			System.out.println("자소서 수정 실패");
		}
